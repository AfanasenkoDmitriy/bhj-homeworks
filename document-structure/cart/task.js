...
productControls.forEach((control) => {
  const buttonAddValue = control.querySelector(
    ".product__quantity-control_inc"
  );
  const buttonRemoveValue = control.querySelector(
    ".product__quantity-control_dec"
  );
  const productQuantityValue = control.querySelector(
    ".product__quantity-value"
  );

  buttonAddValue.addEventListener("click", () => {
    productQuantityValue.textContent =
      parseInt(productQuantityValue.textContent) + 1;
  });
...
const deteleProductFromCartButtons =
  document.querySelectorAll(".product__delete");
